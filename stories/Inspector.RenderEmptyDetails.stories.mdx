...
<Meta title="Inspector (renderEmptyDetails)" component={Inspector} />

# Inspector (renderEmptyDetails)
## Focus: What to show in the details panel on the right while there is no selection

<Props of={Inspector} exclude={Object.keys(Inspector.propTypes).filter(propName => propName !== "renderEmptyDetails")} />
